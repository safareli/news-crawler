...
var sites = Object.keys(profiles).reduce(function(sites,profile){
	return sites.concat(profile.sites);
},[]).filter(onlyUnique).reduce(function(sites,siteName){
	sites[siteName]= new Site(require(path.join(process.cwd(),DIR.sites,siteName,'.js')))
	return sites;
},{});

console.log('sites => ',Object.keys(sites));

Q.all(Object.keys(sites).forEach(function(siteName){
	...
	console.log('aq unda mqondes linkebi :d');
});
